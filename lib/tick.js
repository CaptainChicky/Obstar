/*
	...
	itself runs at. config.REF_TICK_MS (40) is the tick every raw gameplay constant
	...
	pass. Moving the reference from that
	...
	A leaf module - no lib/runtime.js involvement, no circular-graph rule to worry about - so it is
	just require()'d, like public/SHARE/Physics.js.
*/
...
/* Anything added to a velocity/angle/accumulator once per reference tick: accelerations,
	 ...
	 Also - narrower than it looks, and easy to misuse (PENDING #43) - a ONE-SHOT velocity impulse
	 added into a `vec` whose own position step integrates that vec directly with no separate
	 dtTicks multiply of its own: entities/Bullet.js's muzzle kick / 'god' repulsion / collision
	 knockback, entities/Objects.js's collision knockback (both bodies do `x += vec.x` every real
	 tick, nothing else). There the SCALE factor perTick applies IS the tick-rate correction - it is
...
/* A one-shot velocity impulse added directly into a body's `vec` where that body's position is
	 ...
	 finer instead of staying put - PENDING #43, found live: replaying `back = 1.12` (Basic's 0.4 gu)
	 gave 0.2546 gu via the old perTick() wrapping against 0.4073 gu flat, at the live 25ms tick.
	 The only current consumers are entities/Player.js's recoil (`back`) and its four collision
	 knockbacks (including `weight`) - anything that reaches Physics.stepBody, not entities/Bullet.js
	 or entities/Objects.js's own hand-rolled tails, which integrate vec into position directly and
	 correctly keep perTick() for the same shape of impulse (see perTick()'s own comment above). */
...
/* Accumulators that integrate twice over ticks (the accumulator itself grows perTick, then is
	 ...
	 under- or over-shoots by 1/SCALE, so this needs SCALE squared instead. Player.js's regen used
	 to be a third (hpregan), before PENDING #17 replaced the accumulator with two direct perTick()
	 rates and removed the need for this category there. */
