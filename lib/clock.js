/*
	...
	What it replaces. Every room used to end its own update() with
	...
	(speeds, FRICTION, reload counters - see lib/tick.js) and converts them to
	real steps once, at the point of use - a step now means a fixed config.TICK_MS of wall clock
	on average instead of "however long the last one took plus twenty".
...
// ms per simulation step. Set in lib/config.js, which explains at length why it is 25 (40Hz,
// diep.io's own real rate) rather than the ~33ms/29Hz the old setTimeout chain actually achieved
// under load, and how config.REF_TICK_MS/lib/tick.js keep every gameplay constant's real-world
// meaning unchanged across that move.
