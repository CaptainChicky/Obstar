...
const DEFAULT_STEP    = 20;   // ms per simulation step -> 50Hz, the rate the entity code assumes
const DEFAULT_CATCHUP = 5;    // at most 5 steps (100ms) of arrears paid back in one wake-up
...
class Clock {
  ...
  wake(){
    ...
    this.last   = t;
    ///
    let budget = this.stepMs*this.maxCatchup;
    if(elapsed > budget){
      let stall = elapsed;
  ...
  rate(){
    let ms = this.now()-this.startedAt;
