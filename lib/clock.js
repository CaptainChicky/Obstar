/*
	...
	A naive re-arming setTimeout(step, 20) is not a fixed schedule: it means "in *at least*
	20ms", so a tick that took 9ms of CPU comes back 29ms later, not 20ms later, and the error
	never gets repaid. Under load the whole simulation would quietly run slow - entities moving
	at the wrong speed in wall-clock terms, and stutter for anyone whose client smooths toward
	what the server last said. With several rooms open the drift would be independent per room.

	The fix is the standard accumulator: measure how much real time actually passed, and pay
	out that time in whole fixed-size steps. Entity code keeps its per-*reference*-tick constants
	(speeds, FRICTION, reload counters - see lib/tick.js) and converts them to real steps once,
	at the point of use - a step means a fixed config.TICK_MS of wall clock on average, not
	...
	Send rate is deliberately not on this clock. net/gameSocket.js sends at its own rate per
	socket: what a client receives is a snapshot of whatever the simulation had reached, and
	neither rate has to be a multiple of the other.
...
// ms per simulation step. Set in lib/config.js; config.REF_TICK_MS/lib/tick.js keep every
// gameplay constant's real-world meaning unchanged if this ever moves.
...
class Clock {
	...
	wake() {
		...
		if (elapsed > budget) {
			...
			elapsed = budget;
			/*
				Log the stall. A slow simulation looks identical to a laggy network from a
				player's or operator's perspective, so this is the only signal that distinguishes
				them. Throttled to one line a minute so a struggling box does not drown its own log.
			*/
