...
// the choice is deliberate. See the drift call site for why.
...
// : `POSSESSION_TIMER = tps x 60 x 5` (tps=25, its own 40ms tick) - 7500
// reference ticks, 5 minutes despite that file's own comment saying "10 minutes" (a stale
// comment against its own literal). Dominator
...
// `ai.viewRange = 2000` du - x 0.56 = 1120 units. A plain radius,
...
// AI.PASSIVE_ROTATION (0.01 rad per reference tick) and the radius its idle mouse
...
// tank-speed ceiling with room to spare - Closer.txt: faster than every
...
// two were split and RECOMPUTED, not just re-verified, when BODY_FRICTION moved
// 0.956532 -> 0.9: the pre-Step-9 pair gave
...
// re-scanning. diep_wiki gives no number for this - ours, 75 reference ticks = 3s at
// the 40ms reference.
...
function dominatorCapture() {
	// A flip force-ejects whoever's piloting it (diepcustom Dominator.ts's
	...
	if (killerTeam !== null && killerTeam !== this.team) {
		...
		this.domIdle = 0;
		// diepcustom Dominator.ts's own onDeath() notification - only
...
function dominatorUpdate() {
	...
	/*
		diepcustom's real Dominator has no per-barrel auto-turret - the whole
...
/*
	Shared by every real diep boss - diepcustom's own AI.findTarget()
	...
	Defender is the one boss that never calls this at all (`ai.viewRange = 0`, it
...
/*
	diepcustom AbstractBoss's own BossMovementControl: patrol between the four
...
// diepcustom AbstractBoss.ts:204 - a flat maxHP/25000 regen every tick,
...
function bossMotionSummoner() {
	...
	// BODY_FRICTION, not the tank's 10/11, and this is the deliberate call 
...
// detected) - steers and faces straight at the nearest one, AI-driven chase
...
// the Summoner-only `up.BPene` balance hack (no diep basis - the new bosses' `pene`
...
/*
	Guardian's own update - identical to
...
const CONFIG = {
		'BOTS': [
			function () {
				...
				// something *else* had moved stillLvl - which, before was the level-18
		...
		'BOT_PATHS': [
			...
			},
			{
				...
				// Machine Gun in tree rewrite (diep has no Assassin->Sprayer edge),
		...
		/*
			...
			than 7 times - both are the diep economy adopted (they were 27 entries
		...
		*/
		...
		/*
			Tag's win-condition NPC/startClosing()).
			...
			No steering/turn-rate: diep gives it no tank body to be faithful to, and
			 Closer.txt's "immediately go after players" and "ramming into them"
			...
			[
				function () {
					...
					if (best) {
						...
						// freeze in place. "they'll be spinning and slowly drifting in
				...
				function () {
					...
					// - TanksConfig.js's Arena Closer cannon is otherwise an
...
/*
	Mothership - a stationary-until-piloted Player
	...
	onto every cannon, which also blocked a pilot from ever holding fire).
...
function mothershipUpdate() {
	...
	if (this.destroy > 1) {
		...
		this.size *= tick.drag(1.1); // diep's own DeletionAnimation.scale(1.1), Object.ts
		return;
	}
