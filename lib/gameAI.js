...
// One-time-rescaled from the 33ms reference (massplanchunks WP3), then converted to a real-tick
// value once at load - same treatment as entities/Player.js's identical auto-turret lead and
// bot spin-flip chance.
...
const CONFIG = {
		'BOTS': [
			function () {
				...
				///
				// Bots are Players too - same OOB margin as a human's motion() (massplanchunks WP5).
		...
		'PETS': [
			function (play) {
				...
				////
				this.dir = Math.atan2(play.y + play.vec.y * 2.475 + this.pos.y - this.y, play.x + play.vec.x * 2.475 + this.pos.x - this.x);   // 3 one-time-rescaled (lead)
