/*
  ...
  them is the tank / boss / pet, not this module. They close over engine pieces that
  lib/boot.js owns (Detector, Vec, FRICTION, CLASS), so this is a factory: call it once at
  boot, after the entity classes exist, and it hands back the CONFIG object.

  This is the *working* copy of the AI. lib/config.js used to carry a second, diverged
  copy that never executed - see the note at the top of that file.
*/
const KIND = require('./kinds.js');

module.exports = function createGameAI(deps){
  const Detector = deps.Detector;
  const Vec      = deps.Vec;
  const FRICTION = deps.FRICTION;
  const CLASS    = deps.CLASS;
  const DES      = deps.DES;
    ...
    'BOTS':[
      function(){
        ...
        if(this.shield && this.xp<25000){
          ...
          this.inputs.e = 0;
          return;
          // CLEANUP(HANDOFF §12): unreachable after the return above. Either the return is
          // wrong or this dir-randomisation is dead; left as-is (behaviour-preserving) pending
          // a decision. Do not "fix" by moving the return without checking intent.
          if(!this.dir){
        ...
        switch(this.botMod){
          case 'farm':{
            ...
            let selected = 0;
            for(let obj of this.DETEC.selectAll[KIND.OBJECTS]){
          ...
          case 'run':{
            ...
            let x = 0;
            let y = 0;
            for(let bull of this.DETEC.selectAll[KIND.BULLET]){
            ...
            for(let bull of this.DETEC.selectAll[KIND.PLAYER]){
              let dis = this.screen/Math.sqrt(Math.pow(this.x-bull.x,2)+Math.pow(this.y-bull.y,2)*2)/CONFIG.botThreshold.runDis;
          ...
          case 'attack':{
            ...
            let dir = this.spin*Math.PI*Math.min(1,100/dis)/2.5 + this.autoDir;
            break;
          };
          case 'search':
          default:{
      ...
      [
        function(){
          if(!this.DETEC){
            ...
            this.DETEC.y = this.y;
            this.detected = [];
            for(let n of this.DETEC.selectAll[KIND.PLAYER]){
