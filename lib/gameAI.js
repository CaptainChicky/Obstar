        ...
        function(){
          if(!this.DETEC){
            ...
            for(const n of this.DETEC.selectAll[KIND.PLAYER]){
              const dis = Math.sqrt(Math.pow(n.x-this.x,2)+Math.pow((n.y-this.y)/0.5625,2))/n.level;
              if(dis<n.screen/30){
          ...
          const motion = new Vec(.15,0).rotate(Math.atan2(this.y,this.x) - Math.PI*Math.min(1,(dis/this.map.width)));
          this.vec.add(motion)
          this.vec.x*=FRICTION;
          this.vec.y*=FRICTION;
