...
/*
	Mothership (plan.md G1/X1's own scaffolding) - a stationary Player spawned by
	rooms/Mothership.js's createMothership(), the same rebind-at-spawn pattern as
	CONFIG.BOSS/CONFIG.CLOSER/CONFIG.DOMINATOR above. diepcustom's Mothership.ts spins its own aim
	in place while unpossessed (this engine never implements possession - PENDING #2's own
	unbuilt Dominator-piloting design is the closest precedent, and Mothership piloting is out of
	this pass's scope too) - Defender boss's own "stand still and spin" pattern is the same shape,
	reused via bossThrust(this, null) rather than a third copy.
*/
...
function mothershipUpdate() {
	this.hit = Math.max(0, this.hit - 1);
	this.motion();
	this.shoot();   // every cannon is auto:1 - see TanksConfig.js's own Mothership entry
}
