/*
	Maze layout generator: plant scattered seeds, grow each into a branching/turning corridor of
	wall cells, sprinkle a few singular walls, flood-fill from a corner to find (and fill in)
	unreachable pockets, then merge the resulting wall cells into the largest possible
	rectangles (convertToWalls()). Operates purely in grid-cell space; rooms/Maze.js does the
	grid-to-world scaling and Wall instantiation.

	convertToWalls()'s get()/set() do no bounds check, so a chunk growing exactly to the grid
	edge reads into the next row rather than stopping cleanly - this is accepted rather than
	hardened, since fixing it would change which maze shapes come out.
*/
...
class MazeGenerator {
	...
	/* Builds the maze - seeds, growth, sprinkle, flood-fill, pocket-fill. */
	generate() {
		const size = this.config.size;
		const seeds = [];
