...
class Main {
	...
	askConnection(data, ip) {
		...
		const connect = (data, ip) => {
			...
			// the rooms that actually exist. ROOMS (rooms/index.js) is the one list of modes, so
			...
			/////
			// Was `!(0 <= data.name.length <= this.maxPseudoLength)`, the same chained
			// comparison that made checkLength a no-op: it parses as
		...
		if (db.enabled && config.DB.ACC) {
			db.query('SELECT * FROM acc WHERE userkey = $1', [data.key]).then((user) => {
				...
				///
				connect(data, ip);
				// Account flags ride along for free from the SELECT above - this is what lets the
				// console gate on an account's devlevel instead of a typed dev-console password
				//.
	...
	command(id, com) {
		...
		if (com[0] === 'connect') {
			...
			// No more password: an account's devlevel (fetched once at askConnection, alongside
			// everything else in the acc row) is the whole permission check now. A regular
			// player's devlevel is 0, so this just refuses them - same shape as before, no new
			// attack surface.
			...
			switch (this.clients[id].dev) {
				case 3: {
					...
					switch (com[0]) {
						...
						// PENDING #27 - Sandbox spawnability, the same '`;`-god'/'summonRandBoss' pattern
						// this file already uses for testing a mode's own special entities without a
						// real match. `com[1]` optionally names a variant ('destroyer'/'gunner'/
						// 'trapper'); omitted, createDominator() itself picks one at random. Spawns at
						// the room's own centre (0,0) - fine for a Sandbox test, meaningless for a
						// score/position in a real Domination match.
	...
	disconnect(id, ip) {
		...
		// Batch F: if this socket was piloting a Dominator/Mothership, hand the boss back before
		// the body is torn down - otherwise its `pilotedBy` dangles at a removed husk and the boss
		// sits frozen and un-retakeable for the rest of the match.
