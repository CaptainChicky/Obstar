...
class Main {
	constructor() {
		...
		if (db.enabled && config.DB.DEV) this.devs = {};
	...
	command(id, com) {
		...
		if (com[0] === 'connect') {
			...
			if (!com[1]) {
				return 'usage: connect <password>';
			}
			return db.query('SELECT * FROM devs WHERE password = $1', [com[1]]).then((result) => {
				if (result.length) {
					this.devs[result[0].password] = result[0].level;
					this.clients[id].dev = result[0].password;
					return `connected (level ${result[0].level})`;
				}
		...
		if (!this.clients[id].dev || !this.devs[this.clients[id].dev]) {
			return 'insufficient permissions - connect <password> first';
		}
		{
