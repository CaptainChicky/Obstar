	...
	askConnection(data, ip) {
		...
		if (db.enabled && config.DB.ACC) {
			db.query('SELECT * FROM acc WHERE userkey = $1', [data.key]).then((user) => {
				...
				if (!brokenKey && typeof RT.Controller.clients[clientId] === 'object') {
					RT.Controller.clients[clientId].devlevel = user[0].devlevel || 0;
					RT.Controller.clients[clientId].username = user[0].username || null;
				}
				///
			});
		} else {
