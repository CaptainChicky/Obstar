/*
  ...
  Rooms are looked up in RT.ROOMS, which lib/boot.js fills from rooms/. The `server` map has
  slots for '4team' and 'boss' that no room class implements yet, which is why those buttons
  are marked deactivated in views/index.ejs.
*/
...
const USERS      = config.MYSQL ? require('mysql').createPool(require('./dbConfig.js').info) : 0;
...
class Main {
  ...
  newServer(gameMode){
    ...
    // '4team' and 'boss' have a slot in this.server but no class, so they resolve to
    // undefined here - which is why askConnection refuses them before it ever gets this
    // far. Writing rooms/S4team.js and naming it below is all it takes to finish one.
    let s = RT.ROOMS[gameMode] ? new RT.ROOMS[gameMode](this.server[gameMode].length) : undefined;
    if(!s){
      console.log(cc.Bright+cc.BgRed+':NO SUCH GAMEMODE //'+cc.Reset+' '+gameMode);
      return;
    }
