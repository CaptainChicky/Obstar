/*
	...
	`oId` stays an integer slot index - it travels the wire as a uint16 (SocketSchema.js) and
	fixed bot/boss slot ranges depend on it. Allocation, the numeric tombstone
	(tick.KEEP_PLACE), and live-only iteration live behind this class rather than being
	hand-rolled with `!isNaN(obj)` guards at every call site.
	...
	entities is built with `maxIndex: N - 1`.
*/
...
class SlotMap {
	constructor({ maxIndex } = {}) {
		...
		// Sorted live-key cache: live()/entries() would otherwise re-derive
	...
	/*
		...
		stable across churn, so it stays. Cached now - see _sortedKeys' own
