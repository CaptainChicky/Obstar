...
exports.config = {
	...
	*/
	...
	'BASE_DRONE_HP': 2000,     // the wiki's stated pool, taken literally - the MH0-dependent
	// alternative (~6400) is the same figure on diep's own HP scale
	...
	*/
	...
	'BASE_DRONE_CHASE_SPEED': 21.087,     // per ref tick -> 527.2 u/s, and also the return-to-ring
	...
	// motion()/shoot() recurrence over every reachable class (Sniper L15 riding its own recoil,
	// 527.2 u/s - PENDING #15/#16's reload and recoil fixes moved the ceiling from Fighter's 399.2,
	// #14's multiplicative level/stat movement form moved it again to 427.2 then 435.1, #30's
	// 45-level/7-point economy moved it DOWN to 423.7 (the tier gates are every 15 levels now, so
	// the cheapest level a tier-1 class can be ridden at went 9 -> 15 and the level penalty
	// (MOVE_LEVEL_DIV^level) is charged over six more levels before the recoil rider gets going),
	// plan.md step 2's tank magnitudes moved it to 501.7, and step 3's `back` rescale (x1.9143,
	// the column is diep's gu table x 28 x (1-F)/F now) moved it here. Both of those last two are
	// SMALLER jumps than they look: step 2 raised the plain walk 1.28x but the ceiling only 1.18x
	// because `back` had not moved yet, and step 3 nearly doubled `back` but moved the ceiling only
	// 1.051x, because at the maxed-Movement L15 this build is measured at, recoil is a small
	// addition to an already-fast walk (473.8 u/s of the 527.2 is the walk) - and because a
	// once-per-shot impulse loses a further ~0.64x to tick.perTick() at the live 25ms tick. See
	// PENDING #43: that last factor is a wrong lib/tick.js category, not a property of recoil.
	// The user's spec is EXACTLY that ceiling: not faster, so lapping a base in the fastest tank
	// stays survivable; not slower, so nothing outruns a drone on straight-line speed.
	// test/rooms.js fails if the two drift apart.
	'BASE_DRONE_CHASE_TURN': 0.35145,    // rad per ref tick = 8.79 rad/s, used instead of
	// BASE_DRONE_TURN whenever a drone is chasing, and as the TOP of the return's turn blend.
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 527.2 u/s
	// dash: 527.2/60 = 8.79. It rides the same smoothstep k as the return's speed blend, which is
	...
	// constructor default), read by entities/Objects.js and entities/Player.js in place of a base
	// drone's own `pene` - which is a 2000-point health pool, not a penetration value, and reads as
	// a 400x damage multiplier if taken at face value.
