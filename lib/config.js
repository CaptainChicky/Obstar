...
exports.config = {
	...
	*/
	...
	'BASE_DRONE_CHASE_SPEED': 17.09,      // per ref tick -> 427.25 u/s, and also the return-to-ring
	...
	// 427.2 u/s - PENDING #15/#16's reload and recoil fixes moved the ceiling here from Fighter's
	// 399.2). The user's spec is EXACTLY that ceiling: not faster, so lapping a base in the
	// fastest tank stays survivable; not slower, so nothing outruns a drone on straight-line speed.
	// test/rooms.js fails if the two drift apart.
	'BASE_DRONE_CHASE_TURN': 0.285,      // rad per ref tick = 7.13 rad/s, used instead of
	// BASE_DRONE_TURN whenever a drone is chasing, and as the TOP of the return's turn blend.
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 427 u/s
	// dash: 427/60 = 7.13. It rides the same smoothstep k as the return's speed blend, which is
