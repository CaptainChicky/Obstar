...
exports.config = {
	...
	*/
	...
	// a polygon boss.
	/*
		Idle tank-drone orbit (Overseer/Overlord/Manager/Factory/Hybrid with no target). Radii are
		multiples of the owner's own body radius so a big class keeps its swarm proportionally
		clear of itself. Separate table from BASE_DRONE_* above - this orbits a moving owner
		instead of a fixed base post, so it is its own small state machine (entities/Bullet.js's
		droneIdleSteer()). Every drone orbits the same way, slowly, always counterclockwise.

		The cruise figure is a FRACTION of the drone's own maxspeed thrust, not an angular rate:
		the orbit is steered as a velocity field (tangent, leaned in or out by the radial error),
		so the angular speed is simply whatever the drone's own cruise velocity over R works out
		to and can never be asked for more than the drone can fly.
	*/
	'TANK_DRONE_ORBIT_R': 4.5,      // level 2 (home), x owner size.
	'TANK_DRONE_ORBIT_GAP': 1.6,    // radius step per level, x owner size. Three levels, weighted
	// [1, 2, 1] by entities/Bullet.js's rerollTankOrbitLevel() so the middle ring stays busiest.
	'TANK_DRONE_ORBIT_SPEED_FRAC': 0.32,   // steady-state cruise thrust, x the drone's own
	// maxspeed - lands within reach of BASE_DRONE_ORBIT_SPEED's own tangential cruise while staying
	// well under 1, so the ring is a steady drift rather than a race.
	'TANK_DRONE_ORBIT_TOL': 0.35,   // |radial error| / R within which the drone cruises rather
	// than sprinting to close the gap.
	'TANK_DRONE_ORBIT_LEAN': 1.8,   // how hard the drone turns toward or away from the owner per
	// unit of radial error - 1/1.8 of a radius off the ring already means flying straight at it.
	'TANK_DRONE_LEVEL_RELAX': 125,  // ref ticks = 5s between level re-rolls.
	// Swoosh: every CROSS ticks a drone cuts across its own orbit to the far side instead of
	// following the ring round, at CROSS_SPEED_MUL x its cruise speed (never past the drone's own
	// maxspeed - see droneIdleSteer()).
	...
	'TANK_DRONE_CROSS_SPEED_MUL': 2.2,
	'TANK_DRONE_CROSS_LEAD': 0.2,   // fraction of pi the exit phase leads the entry's antipode by -
	// bigger values rejoin the ring more smoothly but bend the swoosh further out from the owner.
	'TANK_DRONE_CROSS_MAX': 100,    // ref ticks - hard ceiling on one swoosh, so a drone knocked
	// off course mid-crossing rejoins the ring instead of hunting a point it has flown past.
