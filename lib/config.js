...
exports.config = {
	...
	'DES': 6,   // diep's own deletion animation is 6 reference ticks (Object.ts's DeletionAnimation, plan.md C1)
	'DEAD_DELAY': 124,   // the nb off ms before the person can replay
	'KEEP_PLACE': 17,
	'SIZE_GET_POS': 40,
	// diepcustom/src/config.ts's factorySpawnChance - odds a team-mode respawn comes out of a
	// living teammate's Factory muzzle instead of the mode's own base placement (rooms/Room.js's
	// factorySpawnPoint(), Arena.ts's attemptFactorySpawn).
	...
	*/
	...
	'BASE_DRONE_HP': 2000,
	// diep's own base-drone damagePerTick, `BaseDrones.ts`'s `damage: 1` run through the same
	// `(7 + 3P) x bullet.damage` formula an ordinary bullet uses at P=0 (plan.md chunk 1 D1) - lands
	// on exactly Basic's own can.damage (TanksConfig.js), which is the same anchor for the same
	// reason (a base drone deals bullet-like damage, not a full body ram).
	'BASE_DRONE_DAMAGE': 7,
	...
	*/
	...
	// speed (a return is a chase back to the ring). PENDING nuance 32's pinning rule is RETIRED
	// (plan.md Step 10) - diep's own base drone is not pinned to the fastest tank at all, it runs a
	// flat 54 du/tick = 756 u/s (diepcustom/src/Entity/Misc/BaseDrones.ts, bullet.speed 2.7:
	// `20 x 2.7 x 0.56 = 30.24` units/ref-tick), pinned to nothing. That is now the spec outright,
	// not a ceiling test/rooms.js's fastestTankSpeed() has to track - 756 u/s outruns even a
	// maxed-Movement Sniper's own 546.36 u/s dash, so the "circling a base is a real race" behaviour
	// the old pin protected is gone; BASE_DRONE_DETECT below drops at the same time, so the
	// encounter gets shorter and sharper rather than simply harder (flagged, not pre-tuned back -
	// PENDING #6's browser checklist still needs a human read on the consequence).
	...
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 756 u/s
	// dash: 756/60/25 = 0.504 rad/ref-tick (the same `turn = speed_u_per_s / 60 / 25` identity every
	// prior re-pin used, plan.md Step 10). It rides the same smoothstep k as the return's speed
	// blend, which is what keeps a fast return from swinging wide of its ring. Re-pinned with
	// CHASE_SPEED above and for the same reason - the pair must move together or the radius stops
	// being 60 units.
	...
	'BASE_DRONE_DETECT': gu(60),        // enemy detection range, measured from the drone.
	// RESTORED from gu(18). diep's own figure is `ai.viewRange = 900` du = 504 units = gu(18)
	// (diepcustom/src/Entity/Tank/Projectile/Drone.ts:52), and adopting it was correct on paper
	// and wrong in a browser: diep applies that radius to a drone flying free anywhere near its
	// owner, while ours orbits a fixed ring and only one drone per centre scans at a time
	// (rooms/Room.js's rotateScout()), so the same number leaves a band around every base where
	// an enemy is comfortably inside the drones' reach and nothing reacts. Playtested; gu(60) is
	// the range this base design was actually built around. Flagged as OURS, not diep's.
	'BASE_DRONE_LEASH': gu(90),         // max distance from base before a chase is abandoned - no
	// diep counterpart (diep's single 900 du filter is both the acquire radius and the leash).
	// Left as ours rather than collapsed onto BASE_DRONE_DETECT: our chase is a much longer-range
	// mechanic and collapsing the two changes base behaviour far more than the reference justifies
	// (plan.md Step 10's own recommendation). Flagged, not a diep number.
	...
	// a polygon boss.
	/*
		ONE aggro rule for every drone in the game - a controllable drone (Overseer/Overlord/
		Manager/Necromancer/Hybrid/Overtrapper...), an uncontrollable one (Mothership's AI half,
		a Dominator's), a Factory minion and every boss's own spawner (Summoner/Guardian/Fallen
		Overlord) alike.

		Two things changed, and only two. The radius is measured from the OWNER rather than from
		the drone - a drone's detector only nominates a candidate, so a swarm's real reach used to
		be the orbit ring PLUS the detector's radius, which is what let a big swarm engage things
		most of a screen away. And the "x0.65 if this drone is uncontrollable" split is gone: an
		uncontrollable drone had no business seeing a third less than the controllable one flying
		beside it on the same tank, which is exactly the Mothership/Overtrapper inconsistency.

		What it still does is SCALE with the owner, off the same quantity it always did - the
		owner's own FOV. `screen` is a view WIDTH, so `screen / 2` is the distance from the tank to
		the edge of what its pilot can see, and the fraction below is stated against that: at 0.575
		a drone engages anything within ~57.5% of the way to the screen edge, i.e. comfortably past
		half the visible field but no longer right up against its rim. A wide-FOV Overlord's swarm
		therefore still reaches further than a Sniper-FOV class's, which is the behaviour worth
		keeping - it is only the size and the controllable/uncontrollable split that move.
	*/
	'DRONE_AGGRO_FOV': 0.575,       // x the owner's FOV RADIUS (screen / 2). A level-45 Overlord
	// (screen 2718) gets 781 units where the old rule gave it the full 1359.
	'DRONE_AGGRO_MIN': gu(18),      // floor, applied AFTER the fraction: diep's own drone
	// `ai.viewRange` of 900 du (Drone.ts:52). Bosses carry a deliberately narrow camera (1120, the
	// AbstractBoss default) and would otherwise end up with a 322-unit reach - shorter than their
	// own bodies are wide in some cases. Nothing with a normal tank FOV ever reaches this floor.
	...
	*/
	...
	'TANK_DRONE_LEVEL_GAP': 3.05,   // radius step per level, x DRONE size. BASE_DRONE_LEVEL_GAP is
	// gu(1) = 28 for a BASE_DRONE_SIZE of 9.2, i.e. one drawn side - 28/9.2 restates that spacing in
	// drone-widths so a tank drone's lanes are as far apart, relative to the drone, as a base one's.
	'TANK_DRONE_LEVELS': 5,
	...
	'TANK_DRONE_ORBIT_SPEED_FRAC': 0.37,   // steady-state cruise, x the drone's own terminal
	// speed - an ordinary drone's terminal is ~229 u/s, so this lands on BASE_DRONE_ORBIT_SPEED's
	// own 85 u/s tangential cruise and both kinds of orbit read at the same pace.
	'TANK_DRONE_CROSS_SPEED_FRAC': 0.8,    // the swoosh's nominal peak, same axis. The actual peak
	// is solved per swoosh so the walk lands on a whole tick, exactly as a base drone's is.
