...
exports.config = {
	...
	*/
	...
	/*
		Orbit AI (plan.md WP4). The old carrot-chase steered toward a point walking the ring at a
		rate the drone's own top speed could never catch (2.6x too slow - see entities/Bullet.js's
		case 1.4 header comment for the derivation), so it never actually orbited at the commanded
		rate. The AI now writes the drone's position directly in polar coordinates instead of
		pursuing one.
	*/
	'BASE_DRONE_ORBIT_R': gu(8),        // nominal orbit radius - FourTeam's single ring uses this
	// directly; TwoTeam derives its own off base-strip spacing instead (rooms/TwoTeam.js), since
	// its centres aren't evenly gu(8) apart.
	'BASE_DRONE_ORBIT_R_MIN': 0.45,     // a drone's own radius = nominal * rand in [MIN, MAX] -
	'BASE_DRONE_ORBIT_R_MAX': 1.20,     // basedrones.png's clumpy, non-ring distribution (WP2).
	// Also the band a shape hit (below) is allowed to push a drone's target radius into.
	'BASE_DRONE_ORBIT_SPEED': 4.56,     // per ref tick -> 114 u/s tangential - 2x the old (broken)
	// carrot-chase's actual 56.8 u/s.
	'BASE_DRONE_CROSS_TIME': 20,        // ref ticks = 0.80s - duration of the diameter-cross swoosh
	'BASE_DRONE_CROSS_UNDERSHOOT': 56,  // units the swoosh undershoots the far radius by - 2 drone
	// side lengths (user-measured).
	'BASE_DRONE_SPIRAL': 0.0159,        // per ref tick, tau ~= 2.5s, via tick.smoothing() - how
	// fast a drone's radius relaxes back to its own target between swooshes.
	'BASE_DRONE_HIT_TURN': 1.0472,      // 60 degrees - documented intent behind the tan(60) = 1.732
	// factor baked into entities/Bullet.js's BASE_DRONE_HIT_KICK at module load.
	'BASE_DRONE_HIT_TIME': 8,           // ref ticks = 0.32s - duration of a shape hit's radial kick
	'BASE_DRONE_HIT_COOLDOWN': 12,      // ref ticks = 0.48s - before another shape hit can react
