...
exports.config = {
	...
	*/
	...
	'BASE_DRONE_CHASE_SPEED': 22.369,     // per ref tick -> 559.2 u/s, and also the return-to-ring
	...
	// 559.2 u/s - PENDING #15/#16's reload and recoil fixes moved the ceiling from Fighter's 399.2,
	...
	// plan.md step 2's tank magnitudes moved it to 501.7, step 3's `back` rescale (x1.9143, the
	// column is diep's gu table x 28 x (1-F)/F now) moved it to 527.2, and PENDING #43's fix (recoil
	// and collision knockback moved from tick.perTick() to the new tick.impulse() - they route
	// through Physics.stepBody, which was re-scaling them by dtTicks a second time on top of
	// perTick()'s own SCALE) moved it here: recoil was only ever delivering ~0.64x of what the
	// `back` column states at the live 25ms tick, so undoing that raised the ceiling 527.2 -> 559.2,
	// a 1.061x move for a 1/0.64 = 1.56x fix, because (as with step 3) recoil is a small addition to
	// an already-fast walk at the maxed-Movement L15 build the ceiling is measured at.
	...
	'BASE_DRONE_CHASE_TURN': 0.37282,    // rad per ref tick = 9.32 rad/s, used instead of
	// BASE_DRONE_TURN whenever a drone is chasing, and as the TOP of the return's turn blend.
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 559.2 u/s
	// dash: 559.2/60 = 9.32. It rides the same smoothstep k as the return's speed blend, which is
	...
	// which is a 2000-point health pool, not a penetration value, and reads as a 4x damage
	// multiplier there if taken at face value (entities/Objects.js's `(pene>1)?pene:pene/2` shape).
	// entities/Player.js no longer needs this substitution (PENDING #18): its own pene-based damage
	// multiplier is gone, so there's nothing left for a drone's real pene to blow up.
