...
exports.config = {
	...
	*/
	...
	'BASE_DRONE_CHASE_SPEED': 20.067,     // per ref tick -> 501.7 u/s, and also the return-to-ring
	...
	// 501.7 u/s - PENDING #15/#16's reload and recoil fixes moved the ceiling from Fighter's 399.2,
	...
	// and plan.md step 2's tank magnitudes moved it here. Note that last jump is SMALLER than the
	// plain walk's: base top speed went 284 -> 362.25 u/s (1.28x) but the ceiling only 1.18x,
	// because recoil rides on `back`, which does not scale until plan.md step 3 rescales the column
	// by (1-F)/F. So the recoil rider's premium over a walk fell 1.49x -> 1.38x, and this number
	// moves once more when step 3 lands.
	...
	'BASE_DRONE_CHASE_TURN': 0.33445,    // rad per ref tick = 8.36 rad/s, used instead of
	// BASE_DRONE_TURN whenever a drone is chasing, and as the TOP of the return's turn blend.
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 501.7 u/s
	// dash: 501.7/60 = 8.36. It rides the same smoothstep k as the return's speed blend, which is
