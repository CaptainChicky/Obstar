...
exports.config = {
	...
	*/
	...
	// Not chase-only any more (plan.md WP4.5.13): it is also the TOP of the return's turn blend, which
	// rides the same smoothstep k as the return's speed blend below. It used not to, and that was the
	// pairing this very comment says must never happen - a returning drone ran the 400u/s dash under
	// BASE_DRONE_TURN's 2.5 rad/s, i.e. a 160-unit turn radius against a 224-unit home ring, which is
	// what made a long return swing wide and overshoot instead of curving home.
	...
	// The same k drives the turn limiter (plan.md WP4.5.13), so speed and turn radius can never come
	// apart: v/omega holds at 34-60 units in every state.
	...
	// a drone touched (plan.md WP4.5.0) and, via entities/Player.js, every tank one touched
	// (plan.md WP4.5.11 - 400x contact damage, i.e. dead in a single tick).
	'BASE_DRONE_PROVOKE_MEMORY': 250,   // ref ticks = 10s (plan.md WP4.5.17) - how long an orbit
	// centre keeps chasing a POLYGON boss after that boss last hurt one of its drones. basedrones.txt
	// has base drones defend against the Fallen bosses on sight but "usually don't target
	// Polygon-based Bosses, such as the Guardian, the Summoner, and the Defender, unless those
	// provoke them first via body damage or drone damage". Our only boss is the Summoner, a polygon
	// boss, so without this the whole base rushed it the moment it wandered into detect range.
