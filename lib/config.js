...
exports.config = {
	...
	*/
	...
	'BASE_DRONE_CHASE_SPEED': 16.947,     // per ref tick -> 423.7 u/s, and also the return-to-ring
	...
	// 423.7 u/s - PENDING #15/#16's reload and recoil fixes moved the ceiling from Fighter's 399.2,
	// #14's multiplicative level/stat movement form moved it again to 427.2 then 435.1, and #30's
	// 45-level/7-point economy moved it here: the tier gates are every 15 levels now, so the
	// cheapest level a tier-1 class can be ridden at went 9 -> 15 and the level penalty
	...
	'BASE_DRONE_CHASE_TURN': 0.28245,    // rad per ref tick = 7.06 rad/s, used instead of
	// BASE_DRONE_TURN whenever a drone is chasing, and as the TOP of the return's turn blend.
	// Derived by holding the turn radius (v/omega) at ~60 units, one tank diameter, at the 423.7 u/s
	// dash: 423.7/60 = 7.06. It rides the same smoothstep k as the return's speed blend, which is
	// what keeps a fast return from swinging wide of its ring.
