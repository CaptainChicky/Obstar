...
function scripted(stepMs, maxCatchup) {
	const box = { at: 0 };
	const c = new Clock(stepMs, maxCatchup, function () { return box.at; });
...
console.log('\nself-removal mid-step:');
{
	...
	const c = new Clock(20, 5);
	const survivors = [counter(), counter(), counter()];
	const suicide = { steps: 0, step: function () { this.steps++; c.remove(this); } };
...
console.log('\ncatch-up and drift:');
{
	...
	const chunks = [21, 19, 35, 5, 20, 40, 12, 28, 20, 20, 33, 7, 60, 20, 20, 20, 20, 20, 20, 20,
		20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20];
	let total = 0;
	for (const ms of chunks) { total += ms; s.advance(ms); }
	...
	for (const ms of chunks) { budget += ms; if (budget >= 20) { naive++; budget = 0; } }
