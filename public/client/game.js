...
(function (CLIENT) {
	...
	// One reference tick (40ms) in 60fps-equivalent frames, so per-reference-tick
	// server constants can be applied per-frame scaled by Global.dtFrames.
	...
	CLIENT.Run = function () {
		...
		///
		/*
			Key order is draw order. Walls sit between shapes and tanks/bullets so a
			projectile's death animation draws on top of the wall it hit.
		*/
		const Instances = {
			'Objects': [],
			'Walls': [],
		...
		const User = new function () {
			...
			this.camx = null;
			this.camy = null;
			// Predator zoom. `zooming` is the server's states[4] bit; zoomOff is a separate
			// eased offset from the tank camera to the locked point, added at render so it
			// cannot trip the camera's teleport-snap.
			this.zooming = 0;
			this.zoomOffX = 0;
			this.zoomOffY = 0;
			this.dx = 0;
			...
			this.vy = 0;
			this.scale = 1;
			// Placeholder until the first packet's class arrives.
			this.class = "Basic";
			this.SH = {
				lapse: -1
			};
			...
			this.update = function () {
				/*
					Local input prediction. `predic` is a small offset from the server position
					that responds to WASD on the next frame, then decays as the server catches up.
				*/
				const Ui = General['Ui'];
				const ups = (CLASS[this.class] && CLASS[this.class].ups) ? CLASS[this.class].ups : TanksConfig.defaultUps;
				// `ups` is panel-row order; Ui.upNb is wire order. CONST.UP_ORDER maps them.
				...
				const motionDir = [0, 0];
				// Per reference tick. Physics.stepBody converts to per-frame.
				...
				/*
					Lead covers travel during Interp's one-interval delay plus half the RTT.
					predic velocity is per reference tick. CONST.SIZE*2 is the ceiling.
				*/
				...
				// Eased independently of camx/camy, toward Game.camX/Y while zooming, back to 0
				// when not. Same smoothing constant as the ordinary camera chase.
		...
		CLIENT.initUi();
		///
		// Tank's screen position relative to the true centre, in window pixels. Zero when
		// the camera has caught up. Subtract this instead of assuming the tank is centred.
		// zoomOff is folded in so aiming while zoomed reads the mouse against the tank.
		General['tankOff'] = () => ({
		...
		General['Interact'] = {
			...
			onkeydown: e => {
				...
				switch (key) {
					...
					case 'k':
					case 'o':
					// Sandbox-only cheat keys - '\' cycles class, ';' toggles god mode.
					case '\\':
					case ';':
					...
					case '5': case '6': case '7': case '8': {
						...
						if (Global.inputs.m) {
							// m+digit: fill that stat's bar. enqueue() clamps against the row cap,
							// already spent/queued, and the lifetime budget.
		...
		function Draw() {
			...
			// Render centre is the tank-trailing camera plus the eased zoom offset.
		...
		function Loop() {
			...
			*/
			...
			// Game.timestamp increments once per server tick. 1.515152 = 2 * 25/33 keeps
			// this spinning at a wall-clock rate independent of tick length.
		...
		*/
		...
		General['SetPacket'] = General['SetPacket'] || function (data) {
			...
			for (const category in Instances) {
				// A wall never moves and is never destroyed; it only drops out of a packet
				// because the view buffer is distance-limited. Keep last-known until told otherwise.
			...
			///REST
			{
				for (const CONSTRUC in data.Instances) {
					for (const OBJ in data.Instances[CONSTRUC]) {
						...
						/*
							Apply the packet to the entity, new or not — constructor args are
						...
						*/
						...
						// One server position, timestamped with when the packet landed. A brand
						// new entity needs this as the second sample before it can move at speed.
		...
		General['WS'].onmessage = packet => {
			...
			switch (type) {
				case 'ping': {
					// Probe 1 is our own probe coming back. Time it; it is not a heartbeat.
