(function (exports, platform) {
	exports.pets = [
		...
		(ctx, param, CONST, C) => {
			...
			const $1 = (12 * param.size / CONST.SIZE) / 20
			ctx.beginPath();
		...
		(ctx, param, CONST, C) => {
			///
			const r = (Date.now() / 3200) % Math.PI * 2;
			ctx.rotate(r);
			const $1 = (12 * param.size / CONST.SIZE) / 18
			ctx.beginPath();
			ctx.moveTo(32 * $1, 0)
		...
		(ctx, param, CONST, C) => {
			///
			const r = (Date.now() / 3600) % Math.PI * 2;
			ctx.rotate(r);
			const $1 = (20 * param.size / CONST.SIZE) / 42
			ctx.beginPath();
			ctx.moveTo(52 * $1, 0);
