...
(function (exports, platform) {
	...
	*/
	const CODECS = {
		...
		/* 0..2pi in an int16. */
		angle: {
			enc: (v) => parseInt((v / (Math.PI * 2)) * 65535),
			dec: (v) => (v / 65535) * Math.PI * 2
		},
		/* A 0..1 ratio in a byte. */
		unit: {
			enc: (v) => Math.max(Math.min(parseInt(v * 255), 255), 0),
			dec: (v) => v / 255
		},
		...
		xpMag: {
			enc: (v) => {
				const exp = v ? parseInt(Math.log10(v) / 3) : 0;
		...
		angles: {
			enc: (v) => v.map(x => String.fromCharCode(parseInt((x + Math.PI) / (Math.PI * 2) * 65535))).join(''),
